...
export function Workouts() {
    ...
    React.useEffect(() => {
        async function loadWorkouts() {
            try {
                setLoading(true);
                const data = await fetchWorkouts(session?.access_token!);
                setWorkouts(data);
            } catch (error) {
                console.error(error);
            } finally {
        ...
        <Stack>
            ...
            {isLoading ? (
                ...
                workouts?.map((workout) => {
                    const totalVolume = workout.workoutItems.reduce(
                        (itemAccumulator, item) =>
                            itemAccumulator +
                            item.sets.reduce((setAccumulator, set) => setAccumulator + Number(set.weight), 0),
                        0,
                    );
